...
df = pd.DataFrame(
    columns=["seed", "n_machines", "n_jobs", "epsilon", "branching_rule", "node_selection", "rounding_rule",
             "lower_bound",
...
for n_jobs, n_machines in job_machines_list:
    ...
    for seed in range(seed_min, seed_max + 1):
        ...
        processing_times = [[p] * n_machines for p in np.random.randint(1, 100, n_jobs).tolist()]
        ...
        for epsilon, node_selection_strategy, lower_bound, branching_rule, rounding_rule in tests_to_do:
            ...
            best_solution, X_int, LB, runtime, nodes_explored, nodes_opt, max_depth, terminate = beb.solve(
                processing_times, verbose=0, opt=OPT_exact)
            ...
            df = df._append({"seed": seed, "n_jobs": n_jobs, "n_machines": n_machines, "epsilon": epsilon,
                             ...
                             "rounding_rule": rounding_rule, "lower_bound": lower_bound,
                             "best_solution": best_solution, "best_bound": LB, "runtime": runtime, "depth": max_depth,
                             "nodes_explored": nodes_explored, "terminate": terminate,
                             "number_of_nodes_for_optimality": nodes_opt, "optimal_solution": OPT_exact,
                             "opt_gap": opt_gap(best_solution, OPT_exact)},
                            ignore_index=True)
